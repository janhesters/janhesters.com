...
You want to access your theme throughout your entire app. So, wrap a `ThemeProvider` around your app in your root layout.
...
When you use TDD, you always start by writing an empty function, component, or a 'do nothing' version of the implementation first. This approach lets you write failing tests against an actual implementation, rather than seeing your tests fail simply because the thing you're testing can't be imported as it doesn't exist yet.
...
Run your tests and see that it fails, as expected. It's important to see tests fail before making them pass to ensure they're working correctly.

In the recordings that you see on screen, you see me running a specific test. But that is just for your convenience so in this video it's easier to see the respective test's results. You should keep running your watch script, which runs your tests for you whenever you save your file.
...
1. Request permissions using `Notifications.requestPermissionsAsync()`.
...
Lastly, it passes whether the user has granted permissions to the wrapped component as a prop.
...
describe('useEffectOnce()', () => {
  test('runs the effect exactly once', () => {
...
You can test hooks by using the `renderHook` function from Testing Library. You need to create a mock function using `jest.fn()`. You can run assertions on this mock function. Check that it hasn't been called. Then render your hook and check that it has been callled. Rerender the hook and assert that it hasn't been called again.
...
Let's tie all of these functions together in your React components. You're going to use the display component and container component pattern. display components are responsible for how things look - they are pure functions that take in props and return JSX. container components handle logic and side effects. They contain hooks and are connected to your Redux store.
...
When you're testing a display component, you first need to think about what it should render. This determines the props it receives, which depend on what the component should display.
...
First, create a `createProps` factory function to make it easy to generate the props for the component, allowing you to only override the ones you need for each specific test case.
...
Your second test should check that if the user was asked for permissions but declined, it displays a message telling them to go to their settings to allow push notifications manually.
...
Create a `noop` function to use as a default for function props.
...
First, import necessary modules and components. You'll need `AsyncStorage` for storing some flags, `Notifications` for handling notifications, `router` from `expo-router` for navigation, and `useState` for React state management.
...
2. It retrieves values from `AsyncStorage` to check if the user was previously asked for permissions or if they have started the app after granting permissions.
...
Next, create an empty root saga in `src/redux/root-saga.js`.
...
If at any point during development your app is in a buggy state or behaves differently from this tutorial, it could be because a weird state has been persisted. You can purge the state by calling `persistor.purge()`. But remember to only do this in development.
...
Next, create a higher-order component (HOC) to handle header configurations.
...
Define an empty initial state and a reducer that returns its state unchanged. Additionally, define a selector that should return the reminders array from the state, but it does nothing yet.
...
Then compose the `selectRemindersArray` selector using `values`, which retrieves each value from an object and returns them in an array.
...
Add a test for the `reminderDeleted` action.
...
describe('reminders reducer', () => {
  describe('selectRemindersArray() selector', () => {
    ...
    test('given a reminder deleted action: deletes the item and returns the correct array', () => {
...
2. Your action creators modify your state in the correct way.
3. Your action creators interact correctly with each other (- if you use several to set up the test).
4. Your selectors return the correct value.

You should test selectors with at least two cases: 

1. One with default state, and
2. one with the state you desire.

For the default state, you can call the `rootReducer` without arguments to get the global initial state. You did this in your first test. And again, if you can't call your `rootReducer` without arguments to produce the default state (for example because you're using Redux Toolkit's `createSlice`), you can create and export the `rootState` from your root reducer file.
...
To test the selector's behavior with state, you can simply pass the action as the second argument to your root reducer, as you did in the test for the `reminderDeleted` action.

If your selector depends on several pieces of state controlled by multiple actions, you can create an actions array and reduce over it to build up the state. You will see examples of this in a later tutorial.
...
The `createPopulatedReminder` factory function uses `faker` to generate mock data. The [`faker` package](https://fakerjs.dev/api/) is a JavaScript library that creates large amounts of fake but realistic data for testing and development. It can generate names, addresses, phone numbers, emails, lorem ipsum text, and more. You'll use this factory to create realistic reminder fixtures for your tests.
...
In your second test, ensure that when the reminder item is pressed, the appropriate handler is triggered. Use `screen.getByRole` to get the button associated with the reminder message and `fireEvent.press` to trigger the press event. Then check that the `onPress` handler was called with the correct arguments.
