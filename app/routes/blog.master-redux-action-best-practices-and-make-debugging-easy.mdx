---
title: "Master Redux Action Best Practices And Make Debugging Easy"
...
When I talked to my mentees, they all struggled to get their actions right.
...
Furthemore, he had selectors to read the current input string and the level string.
...
Some action creators take multiple values in their payload. You can make action creators easier to read by using named parameters.  
...
As long as the action creator remains a pure function, you can freely perform data transformations and even use array methods like `.map()`, `.filter()`, `.reduce()`, and others.
...
If you paid attention, you see that he already does many things right. He named a few actions descriptively, which also caused the sagas to have good names. And he was already using actions in multiple places. The `loginClicked` action both modified the state and started the `handleLoginClicked` saga. However, he structured his code in a way that forced him to dispatch too many actions.  
...
Another important best pratice for actions is that it is okay to let multiple reducers handle the same action. Instead of dispatching `showToast`, let the toast reducer also respond to `loginSucceeded`:
...
This pattern sometimes appears, especially with Redux Toolkit's `createAction`, which the third video in this series covers.  
