...
<iframe
  ...
  src="https://www.youtube.com/embed/BnBtzED2-GE?si=MISP-QWofgqyL8LO"
  title="What Is Redux? (Get A Senior Understanding Of How Redux Works)"
></iframe>
...
![Redux Data Flow](https://i.magecdn.com/ada680/cb7c57_redux_data_flow?f=webp,q.90,p.f,l.t)
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-24">
      ...
      <div className="flex items-center justify-center space-x-4">
        ...
        <button
          className="bg-white text-black hover:bg-white/90 inline-flex h-9 items-center justify-center whitespace-nowrap rounded-md bg-white px-4 py-2 text-sm font-medium text-black shadow transition-colors hover:bg-white/90"
          onClick={() => {
...
Now, let's jump ahead and see how to use your updated `createStore` function. You pass `applyMiddleware` as the third argument, and `applyMiddleware` is called with your middleware.

Since `applyMiddleware` returns a function with the same signature as `createStore` - taking in a `reducer` and `initialState`, and returning the store - this preserves the original behavior when middleware is applied. If you call the `createStore` function inside `applyMiddleware` without an `enhancer`, it functions normally.
