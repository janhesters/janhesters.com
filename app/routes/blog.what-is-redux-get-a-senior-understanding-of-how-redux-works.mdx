...
This article is part of a 5-part series on Redux. The first three together will give you a deeper understanding than 98% of the market. And the last two are tutorials where you build production-ready Redux apps.
...
1. `mapStateToProps` is a function that defines how to transform the current Redux store state into component props. In this case, it maps the state to the props of the `Home` page component.
...
Earlier you set up two slices: the example slice and the user profile slice. The first reason to show two slices in this video is to show you `combineReducers`. The second is that because the user profile slice is more complex, you can see the benefits of selectors.
...
The goal of this video is to provide you with a senior-level understanding of Redux. So let's raise the code quality level further. 
...
You can imagine that importing these wrapper functions in every component and compose all the wrappers where you use `dispatch` is inconvenient. And you'd need a new wrapper function for every new requirement.
...
Middleware makes solving the logging problem simple. The `logger` middleware logs the current action and calls the next middleware in the chain. After all middleware has processed the action, logs the store's state and returns the result.
...
I love you very much. Thank you for reading this article. If you've liked it, please follow me on [X*](https://x.com/janhesters) and let me know what you've learned.
