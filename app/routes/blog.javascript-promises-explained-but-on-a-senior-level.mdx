...
But do you truly understand them at a senior-level?

Hi, I'm Jan, the CTO of ReactSquad, and today, you're going to dive deep into JavaScript promises.

Whether you're a beginner or an experienced developer eager to learn some senior secrets, this video is packed with code examples and best practices to help you master promises.
...
You use the `new` keyword to create a promise. The constructor takes a callback, called the  executor function. This executor functions first parameter is the resolve function. You pass to it the value that the promise should resolve to when the operation completed successfully, in this case `'Hello, world!'`.

It's crucial to differentiate between the promise constructor and the promise instance. The promise constructor refers to the `Promise` class, whereas a promise instance is an object created using the `Promise` constructor, here `resolvingPromise`.
...
If you want to see an example for lazy evaluation, check out ["JavaScript Generators Explained, But On A Senior-Level"](https://www.youtube.com/watch?v=FAK1YR4X-8I).
...
Write a function `awaitSynchronous` that awaits a string. Since you're using the `await` keyword, the function becomes a promise returning function.
...
If you've watched "Unleash JavaScript's Potential With Functional Programming", you know that you can compose functions together. If not, you want to watch that video before continuing with the rest of this video because it assumes you are familiar with function composition.

In that video, you learn about the `pipe` function that composes functions in reverse mathematical order.
...
Now go watch the video on screen because the YouTube algorithm thinks it's the perfect video for you to watch next.

I love you very much. Thank you so much for watching. If you enjoyed this video, give it a thumbs up and subscribe to the channel for more advanced JavaScript videos.
